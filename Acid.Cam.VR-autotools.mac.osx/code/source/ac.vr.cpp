/*
 Acid Cam VR written by Jared Bruni
...
void update_surfaces() {
    ...
    if(cap.read(frame)) {
        cv::Mat flipped;
        cv::flip(frame, flipped, -1);
        ac::draw_func[ac::draw_offset](flipped);
        glEnable(GL_TEXTURE_2D);
        glBindTexture(GL_TEXTURE_2D, texture_1_id);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, flipped.cols, flipped.rows, GL_BGR, GL_UNSIGNED_BYTE, (unsigned char*)flipped.ptr());
...
int prog_main(int argc, char **argv) {
    ...
    if(argc == 3 && std::string(argv[1]) == "-i") {
        filename = argv[2];
        std::cout << "Using video file: " << filename << "\n";
    } else {
    ...
    if(!cap.isOpened()) {
        if(filename.size()==0) {
            std::cerr << "Error could not open capture device " << index << ". Is your webcam plugged in?\n";
    ...
    win_width = 1280;
    win_height = 720;
    SDL_Window *window = SDL_CreateWindow("Acid Cam VR - Oculus", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, win_width, win_height, SDL_WINDOW_OPENGL);
