...
void Filter() {
    ...
    SDL_LockSurface(texture1);
    SDL_LockSurface(surface);
    
    for(unsigned int i = 0; i < texture1->h; ++i) {
        for(unsigned int z = 0; z < texture1->w; ++z) {
            unsigned int *num = (unsigned int*)texture1->pixels+offset;
            unsigned char *c_val = (unsigned char *)num;
            unsigned int *num_one = (unsigned int *)surface->pixels+offset;
            unsigned char *cc_val = (unsigned char *)num_one;
            c_val[0] = cc_val[0] * (alpha);
            c_val[1] = cc_val[1] * (alpha);
            c_val[2] = cc_val[2] * (alpha);
            offset++;
        }
    }
    
    SDL_UnlockSurface(texture1);
...
void load_surfaces() {
    ...
    texture1 = SDL_LoadBMP("tex.bmp");
    if(!texture1) {
        std::cerr << "Error loading bitmap.\n";
        exit(0);
    }
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texture1->w, texture1->h,0, GL_BGRA, GL_UNSIGNED_BYTE, texture1->pixels);
    
    surface = SDL_LoadBMP("tex.bmp");
}
...
void update_surfaces() {
    ...
    glBindTexture(GL_TEXTURE_2D, texture_1_id);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, texture1->w, texture1->h, GL_RGBA, GL_UNSIGNED_BYTE, texture1->pixels);
...
void clean() {
    ...
    ovr_Shutdown();
    if(texture1 != 0) SDL_FreeSurface(texture1);
