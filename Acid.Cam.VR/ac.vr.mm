...
void update_surfaces() {
    ...
    if(cap.read(frame)) {
        ...
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, frame.cols, frame.rows, GL_BGR, GL_UNSIGNED_BYTE, (unsigned char*)frame.ptr());
...
int prog_main(int argc, char **argv) {
    ...
    if(!cap.isOpened()) {
        ...
        //alert.informativeText = @"Could not connect to web cam is it plugged in?";
        [alert runModal];
        [alert release];
        exit(0);
    ...
    std::cout << "Acid Cam version: " << ac::version << "\n";
    init_device(window);
    load_surfaces();
    bool active = true;
    while(active) {
